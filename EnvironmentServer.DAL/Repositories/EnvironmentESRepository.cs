...
public class EnvironmentESRepository
{
    ...
    public async Task AddAsync(long id, string esVersion)
    {
        ...
        await Cli.Wrap("/bin/bash")
            .WithArguments($"-c \"docker run -d --name {dID} -p {port}:{port + 100} -p {port + 200}:{port + 300} -e \"discovery.type=single-node\" -it docker.elastic.co/elasticsearch/elasticsearch:{esVersion}\"")
