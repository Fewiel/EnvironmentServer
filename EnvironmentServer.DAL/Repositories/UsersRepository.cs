...
chsh --shell /bin/bash {1}";
        ...
        private async Task SetupChrootForUserAsync(string user)
        {
            ...
            await Cli.Wrap("/bin/bash")
                .WithArguments($"-c \"bash /tmp/chroot_" + user + ".sh /bin/{ls,cat,echo,rm,bash,sh} /usr/sbin/{phpenmod,phpdismod,phpquery} /usr/bin/{php*,unzip,nano,vi,mkdir,zip,tar,chmod,chown,env,mysql,mysqldump,git,expr,sort,find} /usr/share/zoneinfo /etc/hosts\"")
                .ExecuteAsync();
        }
